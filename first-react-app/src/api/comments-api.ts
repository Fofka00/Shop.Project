...
import { mapCommentsEntity } from "../../Server/services/mapping";
import { OkPacket } from "mysql2";
import { COMMENT_DUPLICATE_QUERY, INSERT_COMMENT_QUERY } from "../../Server/services/queries";
import { param, validationResult } from "express-validator";
import { ICommentEntity, CommentCreatePayload, IComment } from "../../Shared/types";
...
commentsRouter.get('/', async (req: Request, res: Response) => {
  try {
    const [comments] = await connection.query<ICommentEntity[] & RowDataPacket[]>(
...
commentsRouter.get(
  ...
  async (req: Request<{ id: string }>, res: Response) => {
    try {
      ...
      const [rows] = await connection.query<ICommentEntity[] & RowDataPacket[]>(
...
commentsRouter.post('/', async (
  ...
  try {
    ...
    const [sameResult] = await connection.query<ICommentEntity[] & RowDataPacket[]>(
