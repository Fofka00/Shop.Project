...
import { mapCommentsEntity, mapImagesEntity, mapProductsEntity } from "../../Server/services/mapping";
import {
...
productsRouter.get('/', async (req: Request, res: Response) => {
  const [productRowsRaw] = await connection.query("SELECT * FROM products");
  res.send(productRowsRaw);
});
...
// затем rows приводите к нужному типу:
    const productsRaw = rows as IProductEntity[];
    ...
    const [commentRows] = await connection.query<ICommentEntity[] & RowDataPacket[]>("SELECT * FROM comments");
    const [imageRows] = await connection.query<IProductImageEntity[] & RowDataPacket[]>("SELECT * FROM images");

    const products = mapProductsEntity(rows as IProductEntity[]);
...
productsRouter.post('/', async (
  ...
  try {
    ...
    const [rows] = await connection.query<IProductEntity[] & RowDataPacket[]>(
...
productsRouter.post(
  ...
  async (
    ...
    try {
      ...
      const [currentThumbnailRows] = await connection.query<IProductImageEntity[] & RowDataPacket[]>(
      ...
      const [newThumbnailRows] = await connection.query<IProductImageEntity[] & RowDataPacket[]>(
